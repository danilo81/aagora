...
function BimMesh({
  ...
  const highlightColor = useMemo(() => {
    return new THREE.Color(element.color).lerp(new THREE.Color('#ffffff'), 0.3).getStyle();
  }, [element.color]);

  // Material and Rendering logic
  const materialProps = useMemo(() => {
    let baseColor = element.color;
    if (isSelected) baseColor = '#3b82f6';
    else if (isMappedToActiveTarget) baseColor = '#10b981';
    else if (isMapped) baseColor = '#059669';
    else if (hovered) {
      const c = new THREE.Color(element.color);
      baseColor = c.lerp(new THREE.Color('#ffffff'), 0.3).getStyle();
    }

    let meshColor = baseColor;
    let meshOpacity = element.opacity ?? 1;
    let meshTransparent = meshOpacity < 1;
    let meshWireframe = renderMode === 'wireframe';
    let meshDepthWrite = true;
    ...
    if (renderMode === 'solid') {
      ...
      meshDepthWrite = false;
    } else {
      // Element-specific overrides (like Glass or individual X-Ray visibility)
      const isXrayElement = element.visibility === 'xray';
    ...
    return { 
      ...
      depthWrite: meshDepthWrite,
      isXray: renderMode === 'xray' || element.visibility === 'xray',
      isGlass: element.material.toLowerCase().includes('glass')
    };
  }, [element, isSelected, hovered, isMapped, isMappedToActiveTarget, renderMode]);

  const { color, transparent, opacity, wireframe, depthWrite, isXray, isGlass } = materialProps;
    ...
    return (
      ...
      >
        {wallType.layers.map((layer, index) => {
          ...
          else if (hovered) finalColor = highlightColor;
      ...
      {elements.map((element) => {
        ...
        return (
          <BimMesh
            ...
            renderMode={isOutside ? 'wireframe' : renderMode}
            wallTypes={wallTypes}
            lineTypes={lineTypes}
            onClick={(e) => {
              e.stopPropagation();
