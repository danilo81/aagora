...
import { FileText, Loader2, Cloud, Download, Box } from 'lucide-react';
...
interface IfcCloudModalProps {
  ...
  fetchDocuments?: (projectId: string) => Promise<ProjectDocument[]>;
}

export function IfcCloudModal({ isOpen, onClose, projectId, onSelect, fetchDocuments }: IfcCloudModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] bg-zinc-950 border-zinc-800 text-white">
        ...
        <div className="mt-4">
          ...
          <ScrollArea className={cn("h-[400px] rounded-md border border-zinc-800 bg-zinc-900/50 p-2")}>
            {loading ? (
