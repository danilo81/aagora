...
interface SidebarProps {
  ...
  setRenderMode: (mode: 'solid' | 'wireframe' | 'xray' | 'material') => void;
...
export function Sidebar({ selectedElement, onClose, renderMode, setRenderMode, onUpdateElement, bimClasses, lineTypes, scaleSettings }: SidebarProps) {
  ...
  return (
    <div className="absolute top-4 right-4 bottom-4 w-80 bg-zinc-900/90 backdrop-blur-md border border-zinc-800 rounded-xl text-zinc-100 flex flex-col overflow-hidden shadow-2xl z-20 pointer-events-auto">
      ...
      <div className="p-4 border-b border-zinc-800">
        <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">View Modes</h3>
        <select
          value={renderMode}
          onChange={(e) => setRenderMode(e.target.value as any)}
          className="w-full bg-zinc-900 border border-zinc-700 rounded px-2 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-blue-500 transition-colors"
        >
          <option value="material">Material (Standard)</option>
          <option value="solid">Solid (Clay)</option>
          <option value="wireframe">Wireframe</option>
          <option value="xray">X-Ray (Transparency)</option>
        </select>
      </div>
              ...
              <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700/50">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center py-1">
                    ...
                    <span className="text-zinc-200 font-medium font-mono">{selectedElement.volume.toFixed(precision)} mÂ³</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-zinc-500">Area</span>
                    <span className="text-zinc-200 font-medium font-mono">{selectedElement.area.toFixed(precision)} mÂ²</span>
                  </div>
                </div>
              </div>
            </div>
