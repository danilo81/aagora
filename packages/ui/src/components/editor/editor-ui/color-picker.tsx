...
function useComposedRefs<T>(...refs: PossibleRef<T>[]): React.RefCallback<T> {
  ...
  return React.useCallback((node: T | null) => composeRefs(...refs)(node as T), refs)
}
...
function VisuallyHiddenInput<T = InputValue>(
  ...
  // Track previous value using state-based derived-state pattern (React Compiler compatible)
  const initialValue = isCheckInput ? checked : value
  const [prevValueState, setPrevValueState] = React.useState<{
    current: T | boolean | undefined
    previous: T | boolean | undefined
  }>({ current: initialValue, previous: initialValue })

  const currentInputValue = isCheckInput ? checked : value
  if (prevValueState.current !== currentInputValue) {
    setPrevValueState({ current: currentInputValue, previous: prevValueState.current })
  }
  const prevValue = prevValueState.previous
...
function ColorPickerTrigger(props: ColorPickerTriggerProps) {
  ...
  return (
    <PopoverTrigger
      ...
      {...(triggerProps as any)}
    >
      <TriggerPrimitive data-slot="color-picker-trigger">
        {children}
      </TriggerPrimitive>
    </PopoverTrigger>
  )
...
interface ColorPickerFormatSelectProps
  ...
  Partial<Pick<React.ComponentProps<typeof SelectTrigger>, "size" | "className">> { }
