...
import { Dropzone, FileWithMetadata } from "@workspace/ui/components/dropzone";
...
export default function ModelsLibraryPage() {
    ...
    return (
        <div className="container mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500 max-w-7xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card border-accent/20 w-fit">
                <div>
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-center justify-between bg-card p-4 rounded-xl border border-muted/50 backdrop-blur-sm">
                <div className="relative flex-1 max-w-md w-full">
                    ...
                    <Input
                        ...
                        className="pl-10 h-10 border-accent/20 text-[12px] font-medium tracking-widest uppercase w-full bg-background"
                        value={searchTerm}
            ...
            {isLoading && assets.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredAssets.map((model) => (
                        <Card key={model.key} className="bg-card border-accent/20 hover:border-primary/50 transition-all group p-0 overflow-hidden flex flex-col justify-between">
                            <CardHeader className="p-0 aspect-video bg-secondary flex items-center justify-center relative border-b border-accent/20 h-44">
                                    ...
                                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 bg-card rounded-md border border-accent/20 backdrop-blur-md" disabled={isDeletingId === model.key}>
                            ...
                            <CardContent className="p-5 space-y-4">
                                ...
                                <div className="pt-4 border-t border-accent/20 flex items-center justify-between gap-3">
