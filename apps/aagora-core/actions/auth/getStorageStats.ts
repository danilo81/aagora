...
export async function getStorageStats() {
    try {
        // Clerk userId is used in libraryFile, projectDocument, and contact tables
        const clerkUserId = await getAuthUserId();
        if (!clerkUserId) return { success: false, error: 'No autorizado' };

        // storageLimit lives in the DB User table — look up by email
        let limitBytes = 1024 * 1024 * 1024; // default 1 GB
        ...
        // All three tables store Clerk userId (user_xxx), not the DB UUID
        ...
        const [projectDocsSum] = await db
            ...
            .where(and(
                isNotNull(projectDocument.userId),
                eq(projectDocument.userId, clerkUserId)
            ));
