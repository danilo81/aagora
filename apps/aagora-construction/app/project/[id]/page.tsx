...
import { ProjectConfig, Project, ProjectItem, ProjectChangeOrder } from '@/types/types';
...
export default function ProjectDetailPage() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [_localLevels, setLevels] = useState<{ id?: string, name: string }[]>([]);
    ...
    const fetchProject = useCallback(async (id: string) => {
        ...
        try {
            ...
            if (found) {
                ...
                if (found.levels) {
                    setLevels(found.levels.map((l: { id: string, name: string }) => ({ id: l.id, name: l.name })));
                }
            } else {
                setProject(null);
            }
    ...
    const totals = useMemo(() => {
        ...
        const currentTotal = (project.items as ProjectItem[]).reduce((acc: number, pi: ProjectItem) => {
            const apu = calculateAPU(pi.item?.supplies || [], project.config || undefined);
            return acc + (pi.quantity * apu.totalUnit);
        }, 0);

        const changeOrdersTotal = (project.changeOrders || []).reduce((acc: number, oc: ProjectChangeOrder) => {
        ...
        <div className="flex flex-col min-h-screen text-primary pb-20">
            <main className="flex-1 p-4 md:p-8 space-y-12">
                ...
                <div className="relative h-[400px] md:h-[500px] rounded-[40px] overflow-hidden border border-white/10 bg-black group">
                    <Image
                        src={getProjectImageUrl(project?.imageUrl || null)}
                ...
                <section className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-12">
                        ...
                        <div className="p-6 bg-card border border-accent rounded-3xl space-y-2">
                            ...
                            <p className="text-xl font-black">{Array.from(new Set((project?.items as ProjectItem[])?.map((i) => i.item?.chapter))).length}</p>
                        </div>
                        <div className="p-6 bg-card border border-accent rounded-3xl space-y-2">
