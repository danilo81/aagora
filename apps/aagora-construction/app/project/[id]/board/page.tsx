...
import { BimTopic, BimTopicStatus, BimDocument, LibraryFile } from '../../../../types/types';
import {
    getBimBoardData,
    upsertBimTopic,
    deleteBimTopic,
...
import { Editor } from '@workspace/ui/components/editor/index';
...
export default function BimDocumentationPage() {
    ...
    const [docInfo, setDocInfo] = useState<BimDocument | null>(null);
    ...
    const [cloudTemplates, setCloudTemplates] = useState<LibraryFile[]>([]);
    ...
    const activeValues = useMemo(() => {
        ...
        return safeGet(drafts, selectedTopic.id) ?? {
    ...
    const updateDraft = useCallback((topicId: string, updates: Partial<{ title: string, content: string, status: BimTopicStatus }>) => {
        setDrafts(prev => {
            ...
            const currentDraft = safeGet(prev, topicId) ?? {
                title: topic?.title || '',
                content: topic?.content || '',
                status: topic?.status || 'in_progress'
            };
            return safeUpdate(prev, topicId, { ...currentDraft, ...updates });
        });
    ...
    const handleSaveAsTemplate = async () => {
        ...
        try {
            ...
            toast({ title: "Error al guardar", description: "Error desconocido", variant: "destructive" });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const fetchTemplatesOnDemand = useCallback(async () => {
        ...
        try {
            ...
            console.error("Error fetching templates on demand:", e);
        }
    ...
    const handlePersistChanges = async () => {
        ...
        try {
            ...
            toast({ title: "Fallo al guardar", description: "Error desconocido al procesar cambios", variant: "destructive" });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleConfirmCreate = async () => {
        ...
        try {
            ...
            toast({ title: "Error de servidor", description: "Error desconocido", variant: "destructive" });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            toast({ title: "Error al eliminar", description: "Error desconocido", variant: "destructive" });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleApplyTemplate = async (templateId: string) => {
        ...
        try {
            ...
            toast({ title: "Error técnico", description: "Error desconocido", variant: "destructive" });
        } finally {
            setIsSaving(false);
            setSelectedTemplate('none');
        }
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            toast({ title: "Error al exportar", description: "Error desconocido", variant: "destructive" });
        } finally {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-100px)] text-primary overflow-hidden ">
            ...
            <div className="flex flex-1 overflow-hidden  ">
                <aside className=" border border-accent/20  flex flex-col shrink-0 overflow-hidden bg-card rounded-lg opacity-99 m-2 max-h-[90vh]">
                    <div className="p-4 border-b border-accent/20  space-y-4">
                        <div className="relative bg-card">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
                            <Input
                                placeholder="Buscar..."
                                className="pl-9 h-9 bg-card border-accent/20 text-[10px] font-bold"
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={selectedTemplate} onValueChange={handleApplyTemplate} onOpenChange={(open) => open && fetchTemplatesOnDemand()}>
                                <SelectTrigger className="h-9 bg-card border-accent/20  text-[10px] font-black uppercase w-full">
                                ...
                                <SelectContent className="bg-card text-primary border-accent/20">
                                    ...
                                    {cloudTemplates.length > 0 && (
                                        <SelectGroup>
                                            <SelectLabel className="text-[8px] text-muted-foreground uppercase font-black tracking-widest mt-2 border-t border-accent/20 pt-2 pl-2">Mis Plantillas en Nube</SelectLabel>
                            ...
                            {filteredTopics.length > 0 ? (
                                filteredTopics.map((topic) => (
                                    <div key={topic.id} className={cn(
                                        ...
                                        selectedTopicId === topic.id ? "border-accent/20 bg-primary/10" : "border-accent/20 bg-card hover:border-accent/20"
                                    )}>
                                        <div
                                        ...
                                        >
                                            <div className="flex items-center gap-2 overflow-hidden">
                                                <div className={cn("h-1.5 w-1.5 rounded-full shrink-0",
                                                    safeGet(drafts, topic.id) ? "bg-amber-500 ring-2 ring-amber-500/20" :
                                            ...
                                            <Accordion type="single" collapsible className="w-full">
                                                <AccordionItem value="subs" className="border-none">
                                                    ...
                                                    <AccordionContent className="p-0">
                                                        <div className="flex flex-col border-t border-accent/20 bg-card">
                ...
                <main className="flex-1 flex flex-col  overflow-hidden relative">
                    {selectedTopic ? (
                        <div className="flex flex-col h-full animate-in fade-in duration-500">
                            <div className="border border-accent/20 bg-card flex flex-col px-8 py-5 shrink-0 rounded-2xl m-2 gap-6 ">
                                <div className="flex flex-row items-end gap-6 w-full">
                                    <div className="flex-1 space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className="text-[14px] font-bold bg-background h-12 border-accent/20 p-0 outline-none focus-visible:ring-0 uppercase tracking-tighter pl-5 w-full rounded-xl"
                                        />
                                    </div>
                                    <div className="w-72 space-y-2">
                                        ...
                                        <Select value={activeValues?.status || 'in_progress'} onValueChange={(val: BimTopicStatus) => updateDraft(selectedTopicId!, { status: val })}>
                                            <SelectTrigger className="h-12 bg-background border-accent/20 uppercase text-[10px] font-black rounded-xl px-4  w-full">
                                                <SelectValue />
                                            </SelectTrigger>
                                ...
                                {Array.from(visitedTopics).map(topicId => {
                                    ...
                                    return (
                                        <div key={topicId} className={topicId === selectedTopicId ? "w-full block h-full" : "w-full hidden h-full"}>
                                            <Editor
                                                editorSerializedState={topic.content ? JSON.parse(topic.content) : undefined}
                                                onSerializedChange={(json) => updateDraft(topicId, { content: JSON.stringify(json) })}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-8 animate-in fade-in">
                            ...
                            <div className="flex flex-row sm:flex-row gap-4 items-center">
                                <div className="space-y-2 w-64 flex flex-row">
                                    ...
                                    <Select value={selectedTemplate} onValueChange={handleApplyTemplate} onOpenChange={(open) => open && fetchTemplatesOnDemand()}>
                                        <SelectTrigger className="h-12 bg-card border-accent/20 text-[10px] font-black uppercase w-full">
                                            <div className="flex items-center gap-2">
                                                <LayoutTemplate className="h-3 w-3 text-primary" />
                                ...
                                <div className="bg-card">
                                    <Button
                                        variant="outline"
                                        className=" border-accent/20 text-[9px] font-black uppercase tracking-widest text-primary hover:bg-primary/10 cursor-pointer"
                                        onClick={() => handleOpenCreateModal(null)}
                                    >
            ...
            <Dialog open={isSaveTemplateModalOpen} onOpenChange={setIsSaveTemplateModalOpen}>
                <DialogContent className="sm:max-w-md bg-card border-accent/20 text-primary">
                    <DialogHeader>
                    ...
                    <div className="py-4 space-y-3">
                        ...
                        <Input
                            ...
                            className="bg-background border-accent/20 font-bold text-sm h-12 uppercase"
                        />
                    </div>
                    <DialogFooter>
            ...
            <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                <DialogContent className="sm:max-w-2xl bg-card border-accent/20 text-primary p-0 overflow-hidden  flex flex-col h-[80vh] max-h-[350px]">
                    <DialogHeader className="p-6 border-b border-accent/20 bg-card shrink-0">
                    ...
                    <div className="flex-1 overflow-hidden p-6 space-y-8">
                        <div className="space-y-3">
                            ...
                            <Input
                                ...
                                className="h-12 bg-card border-accent/20 uppercase font-bold text-sm"
                                placeholder="EJ: PLAN DE EJECUCIÓN BIM (BEP)"
                            />
                        </div>
                    </div>

                    <DialogFooter className="p-6 border-t border-accent/20  shrink-0 flex gap-4">
                    ...
                    <div className="py-6 space-y-4">
                        <div className="space-y-2">
                            ...
                            <Select value={selectedFolder} onValueChange={setSelectedFolder}>
                                ...
                                <SelectContent className="bg-card text-primary border-accent/20 w-full">
            ...
            <Dialog open={isDocInfoModalOpen} onOpenChange={setIsDocInfoModalOpen}>
                <DialogContent className="sm:max-w-md bg-card border-accent/20 text-primary">
                    ...
                    <DialogFooter>
                        <Button variant="outline" className="w-full text-[10px] font-black uppercase tracking-widest border-accent/20 hover:bg-primary/10" onClick={() => setIsDocInfoModalOpen(false)}>Cerrar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
