...
import { Project, FixedAsset, WarehouseMovement, PurchaseOrder, AssetStatus, ItemSupply, ProjectItem, Level } from '@/types/types';
import {
    Package,
    ArrowUpCircle,
    ArrowDownCircle,
...
export default function ProjectWarehousePage() {
    ...
    const [stock, setStock] = useState<Record<string, { currentStock: number }>>({});
    ...
    const [currentWarehouseItems, setCurrentWarehouseItems] = useState<{ rowId: string; id: string; description: string; unit: string; orderedQuantity: number }[]>([]);
    ...
    const [currentExitItems, setCurrentExitItems] = useState<{ id: string; description: string; unit: string; price: number; stock: number; status: string }[]>([]);
    ...
    const fetchProjectData = useCallback(async () => {
        ...
        try {
            ...
            setStock(stockData as Record<string, { currentStock: number }>);
            setMovements(movs as WarehouseMovement[]);
            if (pos.success) {
                setPurchaseOrders(pos.orders as PurchaseOrder[]);
            }
            if (assets.success) {
                setFixedAssets((assets.assets as FixedAsset[]) || []);
            }
        } catch (error) {
            console.error(error);
        } finally {
    ...
    const inventoryItems = useMemo(() => {
        ...
        const items: { id: string, description: string, unit: string, price: number, stock: number, status: 'agotado' | 'bajo' | 'ok' }[] = [];

        const seenSupplies = new Set();
        project.items?.forEach((pi) => {
            pi.item?.supplies?.forEach((is: ItemSupply) => {
                const s = is.supply;
                if (s && s.typology === 'Material' && !seenSupplies.has(s.id)) {
    ...
    const handleOpenAssignAsset = async () => {
        ...
        const filteredAssets = (assets as (Omit<FixedAsset, 'status'> & { status: string })[])
            .filter((a) => !projectAssetIds.has(a.id) && a.status === 'disponible')
            .map((a) => ({
                ...a,
                status: a.status as AssetStatus
            })) as FixedAsset[];
        setAllLibraryAssets(filteredAssets);
    };
                ...
                <body>
                    <div class="header">
                        <div style="display:flex;align-items:center;gap:10px;">
                            ${LOGO_SVG}
                            <div>
                                <h1 style="font-size: 24px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin: 0;">AAGORA</h1>
                                <p style="font-size:9px;font-weight:700;letter-spacing:2px;color:#6b7280;margin:0;text-transform:uppercase">Gestión de Proyectos</p>
                            </div>
                        </div>
    ...
    const handlePrintEntries = () => {
        ...
        const rows = entries.map((m) => `
            <tr>
                <td style="border: 1px solid #ddd; padding: 10px; font-size: 10px; font-family: monospace;">${m.date ? new Date(m.date).toLocaleDateString() : '—'}</td>
    ...
    const handlePOChange = (poId: string) => {
        ...
        if (po && po.items) {
            setCurrentWarehouseItems(po.items.map((i) => ({
                rowId: i.id,
                id: i.supply.id,
    ...
    const handleEntryQtyChange = (rowId: string, qty: string) => {
        setWarehouseEntryOverrides(prev => ({ ...prev, [rowId]: parseFloat(qty) || 0 }));
    };
    ...
    const handleConfirmWarehouseEntry = async () => {
        ...
        const itemsToSave = currentWarehouseItems.map(item => ({
            supplyId: item.id,
            quantity: warehouseEntryOverrides[item.rowId] ?? item.orderedQuantity
    ...
    const getSupplyComputeForLevel = (supplyId: string, levelId: string): number => {
        ...
        for (const pi of project.items) {
            const levelQty = (pi.levelQuantities as { levelId: string, quantity: number }[] | undefined)?.find((lq) => lq.levelId === levelId)?.quantity ?? 0;
            if (levelQty === 0) continue;
            const itemSupply = pi.item?.supplies?.find((s: ItemSupply) => s.supplyId === supplyId);
    ...
    return (
        <div className="flex flex-col min-h-screen text-primary p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                ...
                <TabsContent value="stock" className="space-y-6">
                    {inventoryItems.length > 0 ? (
                        <Card className="bg-card border-accent/20 text-primary overflow-hidden">
                                ...
                                <div className="flex items-center justify-between gap-4 bg-card p-3 rounded-xl border border-accent/20">
                                    <div className="relative flex-1 max-w-md">
                                        ...
                                        <Input
                                            ...
                                            className="pl-10 h-11 bg-card border-accent/20 text-[12px] font-bold uppercase rounded-xl"
                                            value={searchTerm}
                                    ...
                                    <div className="flex items-end justify-end gap-3">
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="outline" className="border-accent/20 bg-card text-muted-foreground font-black text-[10px] uppercase tracking-widest h-11 px-6 rounded-xl hover:bg-accent hover:text-primary">
                                                    <Printer className="mr-2 h-4 w-4" /> Reportes <ChevronDown className="ml-2 h-3 w-3" />
                                                </Button>
                                            </DropdownMenuTrigger>
                                            <DropdownMenuContent align="end" className="bg-card border-accent/20 text-primary p-2 rounded-xl w-60 shadow-2xl">
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {inventoryItems.map((item) => (
                                                <TableRow key={item.id} className="border-accent/20 hover:bg-muted/40 transition-colors">
                        ...
                        <Card className="bg-card border-accent/20 text-primary overflow-hidden ">
                            ...
                            <CardContent className="p-0">
                                <Table>
                                    ...
                                    <TableBody>
                                        {movements.map((m) => (
                                            <TableRow key={m.id} className="border-accent/20 hover:bg-muted/40 transition-colors group">
                                                <TableCell className="py-5 px-8">
                                                    <span className="text-[11px] font-mono font-bold text-muted-foreground uppercase">{m.date ? new Date(m.date).toLocaleString('es-ES') : '—'}</span>
                                                </TableCell>
                                                <TableCell>
                ...
                <TabsContent value="assets" className="space-y-6">
                    {fixedAssets.length > 0 ? (
                        <Card className="bg-card border-accent/20 text-primary overflow-hidden ">
                            <CardHeader className="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0 pb-7 border-b border-accent/20">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-right bg-accent/20 px-6 py-2 rounded-xl border border-accent/20">
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {fixedAssets.map((asset) => {
                                            ...
                                            const sc = statusConfig[asset.status] ?? { label: asset.status, cls: 'bg-accent text-primary border-accent/20' };
                                            return (
                                                <TableRow key={asset.id} className="border-accent/20 hover:bg-accent/30 transition-colors group">
                                                    <TableCell className="py-5 px-8">
                        ...
                        <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-accent/20 rounded-[2rem] bg-card/10 backdrop-blur-sm group hover:border-primary/30 transition-all duration-500">
            ...
            <Dialog open={isWarehouseEntryOpen} onOpenChange={setIsWarehouseEntryOpen}>
                <DialogContent className="min-w-300 bg-card border-accent/20 text-primary p-0 overflow-hidden shadow-2xl flex flex-col h-[85vh]">
                    <DialogHeader className="p-6 border-b border-accent/20  flex flex-row items-center justify-between shrink-0">
                        <div className="flex items-center gap-3">
                    ...
                    <div className="p-6 space-y-6 flex-1 overflow-hidden flex flex-col">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <Select value={selectedPOForEntry} onValueChange={handlePOChange}>
                                    <SelectTrigger className="h-11 bg-card border-accent/20 uppercase font-bold text-xs">
                        ...
                        <div className="flex-1 border border-accent/20 rounded-xl overflow-hidden bg-card">
                            <ScrollArea className="h-full">
                                <Table>
                                    <TableHeader className="bg-accent/20 border-accent/20 sticky top-0 z-10 backdrop-blur-md">
                                        <TableRow className="border-accent/20 hover:bg-transparent">
                                    ...
                                    <TableBody>
                                        {currentWarehouseItems.length > 0 ? (
                                            currentWarehouseItems.map((item) => (
                                                <TableRow key={item.rowId} className="border-accent/20 hover:bg-accent transition-colors">
                                                    ...
                                                    <TableCell className="px-4">
                                                        <Input
                                                            type="number"
                                                            step="0.01"
                                                            value={warehouseEntryOverrides[item.rowId] ?? item.orderedQuantity}
                                                            onChange={(e) => handleEntryQtyChange(item.rowId, e.target.value)}
                                                            className="h-9 bg-card border-accent/20 text-center font-mono text-xs text-emerald-500 font-black"
                                                        />
                                                    </TableCell>
                                                </TableRow>
                                            ))
                    ...
                    <DialogFooter className="p-6 border-t border-accent/20  shrink-0 bg-transparent">
            ...
            <Dialog open={isWarehouseExitOpen} onOpenChange={setIsWarehouseExitOpen}>
                <DialogContent className="min-w-300 bg-card border-accent/20 text-primary p-0 overflow-hidden h-[95vh] flex flex-col shadow-2xl">
                    <DialogHeader className="p-6 border-b border-accent/20 flex flex-row items-center justify-between shrink-0">
                        <div className="flex items-center gap-3">
                                ...
                                <TableBody>
                                    {currentExitItems.length > 0 ? (
                                        currentExitItems.map((item) => {
                                            ...
                                            const availableItems = (project?.items || []).filter((pi: ProjectItem) => pi.item?.supplies?.some((is: ItemSupply) => is.supplyId === item.id));
                                            const selectedPI = availableItems.find((pi: ProjectItem) => pi.item?.id === assignment.itemId);
                                            const availableLevels = project?.levels?.filter((lvl: Level) => {
                                                if (!selectedPI) return false;
                                                const lq = selectedPI.levelQuantities?.find((q: { levelId: string, quantity: number }) => q.levelId === lvl.id);
                                            ...
                                            return (
                                                <TableRow key={item.id} className={cn("border-accent/20 hover:bg-muted/40 transition-colors", (isExceeded || isStockExceeded) && "bg-red-500/5")}>
                                                    <TableCell className="px-6 py-4">
                                                        <div className="flex flex-col">
                                                    ...
                                                    <TableCell className="px-4">
                                                        <Select value={assignment.itemId} onValueChange={(val) => handleExitAssignmentChange(item.id, 'itemId', val)}>
                                                            <SelectTrigger className="h-10 bg-card border-accent/20 text-[10px] font-bold uppercase w-full rounded-lg">
                                                                <SelectValue placeholder="Partida..." />
                                                            </SelectTrigger>
                                                            <SelectContent className="bg-card text-primary border-accent/20">
                                                                <SelectItem value="none" className="text-[10px] font-bold uppercase">Sin asignar</SelectItem>
                                                                {availableItems.map((pi: ProjectItem) => pi.item && (
                                                    ...
                                                    <TableCell className="px-4">
                                                        <div className="flex flex-col items-center gap-1">
                                                            <Input
                                                                ...
                                                                className={cn("h-10 w-24 bg-card border-accent/20 text-center font-mono text-sm rounded-lg font-black",
                    ...
                    <DialogFooter className="p-6 border-t border-accent/20 bg-transparent shrink-0">
            ...
            <Dialog open={isSelectStockForExitOpen} onOpenChange={setIsSelectStockForExitOpen}>
                <DialogContent className="max-w-3xl bg-card border-accent/20 text-primary p-0 overflow-hidden h-[80vh] flex flex-col ">
                    <DialogHeader className="p-6 border-b border-accent/20 bg-accent/2 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-primary/20 rounded-lg">
                                <Boxes className="h-5 w-5 text-primary" />
                            </div>
                    ...
                    <div className="p-6 flex-1 overflow-hidden">
                        <ScrollArea className="h-full border border-accent/20 rounded-xl bg-card">
                            <Table>
                                <TableHeader className="bg-accent/20">
                                ...
                                <TableBody>
                                    {inventoryItems.filter(s => s.stock > 0).map((s) => (
                                        <TableRow key={s.id} className="border-accent/20 hover:bg-muted/40 transition-colors">
                                            <TableCell className="text-center">
                                                <Checkbox checked={selectedStockExitIds.includes(s.id)} onCheckedChange={() => handleToggleStockExitItem(s.id)} className="border-accent/20" />
                                            </TableCell>
                    ...
                    <DialogFooter className="p-6 border-t border-accent/20 bg-accent/20 shrink-0">
            ...
            <Dialog open={isAssignAssetOpen} onOpenChange={setIsAssignAssetOpen}>
                <DialogContent className="max-w-2xl bg-card border-accent/20 text-primary p-0 overflow-hidden flex flex-col shadow-2xl">
                    <DialogHeader className="p-6 border-b border-accent/20 bg-card shrink-0 flex flex-row items-center gap-4">
                        <div className="p-2 bg-primary/20 rounded-lg">
                            ...
                            <ScrollArea className="h-[400px] pr-4">
                                <div className="space-y-3">
                                    {allLibraryAssets.map((asset) => (
                                        <div key={asset.id} className="flex items-center justify-between p-4 bg-accent/10 border border-accent/20/50 rounded-xl hover:bg-accent/20 transition-all group">
                                            <div className="flex items-center gap-4">
                                                <div className="p-2 bg-card border border-accent/20 rounded-lg">
