...
export default function ReportesPage() {
    ...
    const generateOrdersPDF = () => {
        ...
        const html = `
            <html>
                <head>
                    <title>Histórico de Pedidos - ${project.title}</title>
                    <style>
                        body { font-family: 'Helvetica', 'Arial', sans-serif; padding: 40px; color: #333; line-height: 1.4; }
                        .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #000; padding-bottom: 20px; margin-bottom: 30px; }
                        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                ...
                <body>
                    <div class="header">
                        ...
                        <div style="text-align: right;"><p style="font-size: 10px; font-weight: 900; margin: 0;">HISTÓRICO DE PEDIDOS DE OBRA</p></div>
                    </div>
                    <h2>${project.title}</h2>
                    <table><thead><tr><th>Capítulo</th><th>Insumo</th><th>Und.</th><th>Cant. Total</th></tr></thead><tbody>${rows}</tbody></table>
                    <script>window.onload = function() { window.print(); window.onafterprint = function() { window.close(); }; }</script>
                </body>
            </html>
        `;
        ...
        <div className="flex flex-col min-h-screen text-primary">
            <main className="container mx-auto px-4 py-8 space-y-6 flex-1">
                ...
                <Card className="bg-card border-accent/20 overflow-hidden gap-0">
                    <CardHeader className="border-b border-accent/20 bg-card">
                            ...
                            <TableBody>
                                {reports.map((report) => {
                                    ...
                                    return (
                                        <TableRow
                                            key={report.id}
                                            className="border-accent/20 hover:bg-muted/40 transition-colors"
                                        >
                                            ...
                                            <TableCell className="text-right">
                                                <Button
                                                    size="sm"
                                                    variant="default"
                                                    className="border-accent/20 hover:bg-primary/40 text-xs bg-primary text-background"
